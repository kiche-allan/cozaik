...
# This path insert is unavoidable for importing the darknet.py file
sys.path.insert(0, '/content/darknet')
import darknet
...
class Tracked:
    ...
    def update(self, position, other, time, timePassed):
        ...
        self.x = 0.3 * X_t[0][0] + 0.7 * self.x
        self.y = 0.3 * X_t[1][0] + 0.7 * self.y
        ...
        if self.dY != 0:
            self.timeToIntercept = 0.7 * (self.y / self.dY * 0.03333333333333333) + 0.3 * self.timeToIntercept
        self.lastX = self.x
        self.lastY = self.y
        self.lastTracked = time
    ...
    def calcEstimatedPos(self, timePassed):
        if self.lastHistory >= 2:
            dxmin = 0.7 * (self.xmin - self.lastXmin) / timePassed + 0.3 * (self.lastXmin - self.lastX2min) / self.lastTimePassed
            dymin = 0.7 * (self.ymin - self.lastYmin) / timePassed + 0.3 * (self.lastYmin - self.lastY2min) / self.lastTimePassed
            dxmax = 0.7 * (self.xmax - self.lastXmax) / timePassed + 0.3 * (self.lastXmax - self.lastX2max) / self.lastTimePassed
            dymax = 0.7 * (self.ymax - self.lastYmax) / timePassed + 0.3 * (self.lastYmax - self.lastY2max) / self.lastTimePassed
...
def computeDistance(a, b, epsilon=1e-05):
...
class YOLO:
    ...
    def init(self, frame_width, frame_height, timestamp, settings):
        os.chdir(settings.darknetPath)
        metaPath = '/content/yolofiles/cav.data'
        configPath = '/content/yolofiles/yolov4-tiny-cav.cfg'
        weightPath = '/content/yolofiles/yolov4-tiny-cav.weights'
        self.network, self.class_names, self.class_colors = darknet.load_network(configPath,
          metaPath,
          weightPath,
          batch_size=1)
    ...
    def cvDrawBoxes(self, detections, img, timestamp):
        color_dict = {'person':[
         ...
         'bicycle':[
          238, 123, 158], 
         'car':[24, 245, 217],  'motorbike':[224, 119, 227],  'aeroplane':[
          154, 52, 104], 
         'bus':[
          179, 50, 247], 
         'train':[180, 164, 5],  'truck':[82, 42, 106],  'boat':[
          201, 25, 52], 
         'traffic light':[
          62, 17, 209], 
         'fire hydrant':[60, 68, 169],  'stop sign':[199, 113, 167],  'parking meter':[
          19, 71, 68], 
         'bench':[
          161, 83, 182], 
         'bird':[75, 6, 145],  'cat':[100, 64, 151],  'dog':[
          156, 116, 171], 
         'horse':[
          88, 9, 123], 
         'sheep':[181, 86, 222],  'cow':[116, 238, 87],  'elephant':[74, 90, 143],  'bear':[
          249, 157, 47], 
         'zebra':[
          26, 101, 131], 
         'giraffe':[195, 130, 181],  'backpack':[242, 52, 233],  'umbrella':[
          131, 11, 189], 
         'handbag':[
          221, 229, 176], 
         'tie':[193, 56, 44],  'suitcase':[139, 53, 137],  'frisbee':[
          102, 208, 40], 
         'skis':[
          61, 50, 7], 
         'snowboard':[65, 82, 186],  'sports ball':[65, 82, 186],  'kite':[
          153, 254, 81], 
         'baseball bat':[
          233, 80, 195], 
         'baseball glove':[165, 179, 213],  'skateboard':[57, 65, 211],  'surfboard':[
          98, 255, 164], 
         'tennis racket':[
          205, 219, 146], 
         'bottle':[140, 138, 172],  'wine glass':[23, 53, 119],  'cup':[
          102, 215, 88], 
         'fork':[
          198, 204, 245], 
         'knife':[183, 132, 233],  'spoon':[14, 87, 125],  'bowl':[
          221, 43, 104], 
         'banana':[
          181, 215, 6], 
         'apple':[16, 139, 183],  'sandwich':[150, 136, 166],  'orange':[219, 144, 1],  'broccoli':[
          123, 226, 195], 
         'carrot':[
          230, 45, 209], 
         'hot dog':[252, 215, 56],  'pizza':[234, 170, 131],  'donut':[
          36, 208, 234], 
         'cake':[
          19, 24, 2], 
         'chair':[115, 184, 234],  'sofa':[125, 238, 12],  'pottedplant':[
          57, 226, 76], 
         'bed':[
          77, 31, 134], 
         'diningtable':[208, 202, 204],  'toilet':[208, 202, 204],  'tvmonitor':[
          208, 202, 204], 
         'laptop':[
          159, 149, 163], 
         'mouse':[148, 148, 87],  'remote':[171, 107, 183],  'keyboard':[
          33, 154, 135], 
         'cell phone':[
          206, 209, 108], 
         'microwave':[206, 209, 108],  'oven':[97, 246, 15],  'toaster':[
          147, 140, 184], 
         'sink':[
          157, 58, 24], 
         'refrigerator':[117, 145, 137],  'book':[155, 129, 244],  'clock':[
          53, 61, 6], 
         'vase':[
          145, 75, 152], 
         'scissors':[8, 140, 38],  'teddy bear':[37, 61, 220],  'hair drier':[
          129, 12, 229], 
         'toothbrush':[
          11, 126, 158], 
         'cav':[24, 245, 217]}
                        ...
                        if name_tag == 'car':
                            ...
                            if name_tag == 'truck':
                                ...
                                if name_tag == 'bus':
                                    ...
                                    ObjectHeight = 1.8
                    ObjectHeightOnSensor = self.settings.cameraHeight * h / self.settings.imageHeight
                    distancei = self.settings.focalLength * ObjectHeight / ObjectHeightOnSensor
                    if name_key in tracked_items:
                        angle = math.radians((x - self.settings.imageWidth / 2.0) * (self.settings.hFOV / self.settings.imageWidth) + self.settings.cameraAdjustmentAngle)
                        ...
                        crossSection = math.sin(math.radians((xmax - xmin) * (self.settings.cameraHeight / self.settings.imageWidth))) * distancei
        ...
        if self.showImage:
            for detection in self.trackedList:
                if detection.lastHistory >= 1:
                    ...
                    if self.forwardCollisionWarning:
                        if -0.5 <= detection.x <= 0.5:
                            if 0.0 <= detection.timeToIntercept <= 2.5:
                                cv2.rectangle(img, pt1, pt2, [255, 0, 0], -1)
                                cv2.putText(img, str(round(detection.timeToIntercept, 2)), (
                        ...
                        cv2.putText(img, str(detection.id) + ' t:' + str(tracked_items[detection.type]) + ' x:' + str(round(detection.x, 2)) + ' y:' + str(round(detection.y, 2)), (
        ...
        if len(detections_list_positions) > 0:
            if len(self.trackedList) > 0:
                ...
                for dl, dlp in zip(detection_list, detections_list_positions):
                    new = Tracked(dlp[0], dlp[1], dlp[2], dlp[3], dl[0], dl[1], dl[2], dl[3], dl[4], self.time,
                                  self.id)
                    if self.id < 1000:
                        self.id += 1
                    else:
...
class ProcessCamera:
    ...
    def __init__(self, settings):
        self.yolo = YOLO()
        self.yolo.init(settings.width, settings.height, time.time(), settings)

    def processCameraFrame(self, frame_read, cam_time_data):
        coordinates, timestamp = self.yolo.readFrame(frame_read, cam_time_data)
        return (coordinates, cam_time_data)


class Camera:

    def __init__(self, settings):
        self.use_camera = settings.useCamera
        if self.use_camera:
            self.camera = nano.Camera(settings.flip, settings.width, settings.height, settings.fps)
            print('CSI Camera ready? - ', self.camera.isReady())
        else:
            print('Opening captured video file ', settings.inputFilename)
            self.cap = cv2.VideoCapture(settings.inputFilename)
            f = open(settings.camTimeFile, 'r+')
            self.cam_time = f.readlines()
            self.cam_time_idx = 0
            f.close()

    def takeCameraFrame(self):
        if self.use_camera:
            cam_time_data = time.time()
            frame_read = self.camera.read()
        else:
            ret, frame_read = self.cap.read()
            cam_time_data = float(self.cam_time[self.cam_time_idx].split(',')[0])
            self.cam_time_idx += 1
        return (
         frame_read, cam_time_data)
