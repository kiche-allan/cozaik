...
#
# Evaluation Workload: 4-CAV Sensor Fusion
#
# SQ Count: ~36 SQs
# Depth: 4
...
def local_fusion(processed_camera, processed_lidar, cav_num):
    ...
    localization_offsets = [[-.75, 0.0, 0.], [-1.5, 0.0, 0.], [-2.25, 0.0, 0.], [-3.0, 0.0, 0.]]
...
@SQify
def global_fusion(fusion_cav0, fusion_cav1, fusion_cav2, fusion_cav3):
    import sys, os, time
    global sq_state
    ...
    return [results, fusion_cav0[1], fusion_cav1[1], fusion_cav2[1], fusion_cav3[1],
            fusion_cav0[3], fusion_cav1[3], fusion_cav2[3], fusion_cav3[3], vehicle_positions, time.time()]


@SQify
def write_to_file(fusion_result):
    import os, time
    outfile = os.environ.get('TTPYTHON_OUTPUT_FILE', '/tmp/eval_4cav_output.txt')
    results = fusion_result[0]
    vehicle_positions = fusion_result[9]
...
def eval_4cav_fusion(trigger):
    ...
    with TTClock.root() as root_clock:
        ...
        with TTConstraint(name="rsu"):
            global_fusion_result = global_fusion(fusion_result, fusion_result2, fusion_result3, fusion_result4)
