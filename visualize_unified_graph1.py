...
# Infrastructure SQ prefixes to filter out
...
def is_infra_sq(sq_name):
    """Return True if this is an infrastructure SQ to filter out."""
    return any(sq_name.startswith(prefix) for prefix in INFRA_PREFIXES)


def draw_unified_graph(unified_graph, output_path, graph_name):
    ...
    for eligible, sq_list in constraint_groups.items():
        ...
        fill = fill_palette[group_idx % len(fill_palette)]
    ...
    stats = (f"App SQs: {len(sq_to_nodes)} | "
...
def main():
    ...
    draw_unified_graph(unified_graph, output_path, graph_name)
